...
-- thỏa mãn.
-- Đối với bảng KhachHang phải check [MaKhachHang] ,[TenHoKH] ,[TenDemKH]sggd
-- ,[TenKH] ,[NgaySinh] ,[SoDienThoai] ,[DiaChi1] ,[DiaChi2] ,[GioiTinh],as
...
AS
BEGIN
    -- RETURN YEAR(GETDATE()) - YEAR(@NgaySinh)) - (RIGHT(CURDATE(), 5) < RIGHT(@NgaySinh, 5)
    DECLARE @tuoi INT   
...
PRINT N'Tuổi của bạn là: ' + CONVERT(NVARCHAR, dbo.Function_TinhTuoiTD('2000/02/17'))
